...
-   **Gallery** → **💖 Style Pick (스타일 픽)**
-   **Studio** → **✨ Making Room (메이킹 룸)**
-   **My Page** → **📒 Lookbook (룩북)**
-   **Ranking** → **👑 Hall of Fame (명예의 전당)**
-   **Ranking** → **👑 Hall of Fame (명예의 전당)**
-   **Cheering Message (Core Branding):**
    -   Format: `@{User}님이 {Target}를 응원합니다`
...
│   │   │   ├── stylist/
│   │   │   │   └── feedback/
│   │   │   │       └── route.ts        # AI Stylist feedback (Gemini 2.0 Flash)
│   │   │   ├── gallery/
...
│   │   ├── Header.tsx                   # Main Header with Search/Noti
│   │   ├── SearchOverlay.tsx            # Header Search Overlay
│   │   ├── NotificationDropdown.tsx     # Header Notification Dropdown
│   │   ├── BottomNav.tsx                # Mobile Bottom Navigation
│   │   ├── CheeringBadge.tsx            # Cheering Message Component
│   │   ├── StylistFeedbackCard.tsx      # AI Virtual Stylist feedback UI (tabs + lock/unlock)
│   │   ├── studio/
...
│   └── lib/
│       ├── stylist-personas.ts         # AI Virtual Stylist persona definitions & prompts
│       ├── simulation-config.ts        # Agent personas & config
│       ├── firebase/
...
4명의 가상 AI 스타일리스트가 사용자의 디자인을 평가합니다. 각 캐릭터는 빅4 기획사 출신 전직 Creative Director로, 독립 스타일하우스를 운영하는 설정입니다.

### Characters

| ID | Name | Stylehouse | MBTI | Background | Style DNA |
|----|------|-----------|------|------------|-----------|
| `nova` | 차하은 (Cha Ha-eun) | NOVA | INTJ | SN 출신, 광야/SMCU 설계 | Neo-Culture, Chrome, Y3K |
| `onyx` | 윤시혁 (Yoon Si-hyuk) | ONYX | ISTP | YCC 출신, 올블랙/WHO'S NEXT? | Dark Luxury, Power Shoulder |
| `lore` | 서유진 (Seo Yu-jin) | LORE | INFP | HIBE 출신, 화양연화 서사 설계 | Retro-Futurism, Narrative Wear |
| `prism` | 한도윤 (Han Do-yoon) | PRISM | ENFP | JIP 출신, 도파민 스테이징 | Color Blocking, Sporty-Chic |

### IP Risk Mitigation (Dual-Layer Naming)

- **User-facing UI** (bios, stylehouse): Coded names → SN, YCC, HIBE, JIP
- **System prompts** (internal, never exposed to users): Real company names → SM, YG, HYBE, JYP
- **AI Output Filter**: `filterArtistNames()` — 50+ real K-POP artist/group names auto-replaced with "아티스트"
- **Disclaimer**: "AI 버추얼 스타일리스트 · 특정 기획사와 공식 관계 없음" (always visible in UI)
